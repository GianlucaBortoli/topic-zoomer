...
\begin{enumerate}
    ...
    \item check if recomputation of the entire solution can be avoided (see Section \ref{avoidingRecomputation} for a more detailed explanation). If it is possible, proceed from step \ref{init} considering \emph{only} the squares not computed yet. Run the rest of the pipeline from scratch otherwise.
...
\begin{algorithm}
   ...
   $k \longleftarrow$ topics\\
    ...
    \Fn{\CompGrid{}}{
        \tcc{create a new RDD from the input dataset}
        ...
        squares $\longleftarrow$ getSquares$($area, step$)$\;
        \tcc{assign a cell to each web page, then remove punctuation from the web pages, split each web page into an array and insert each web page into a $Row$ data structure that will be put into the $DataFrame$ of the belonging cell and finally group each web page by their $cell\_id$}
        dist\_clust\_pages $\longleftarrow$ dist\_pages.map$(lambda$ page: assign\_cell$($page$))$.
        \linebreak
       ...
       \linebreak
        ...
        \caption{Grid Computation}\label{alg1}
    }
\end{algorithm}
\begin{algorithm}
    \SetKwData{Left}{left}\SetKwData{This}{this}\SetKwData{Up}{up}
    \SetKwFunction{Union}{Union}\SetKwFunction{FindCompress}{FindCompress}
    \SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
    \Input{$dist\_clust\_pages \longleftarrow$ Output Alg\ref{alg1}}
    ...
    \caption{create DataFrame}\label{alg2}
    }
\end{algorithm}
\begin{algorithm}
    \SetKwData{Left}{left}\SetKwData{This}{this}\SetKwData{Up}{up}
    \SetKwFunction{Union}{Union}\SetKwFunction{FindCompress}{FindCompress}
    \SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
    \Input{$clust\_dFs \longleftarrow$ Output Alg\ref{alg2} \\
   $k \longleftarrow$ topics}
    \Output{save topics to HDFS}
    \Fn{\CompTop{cell\_id, cell\_df, k}}{
  ...
  \tcc{load German stopwords}
    ...
    \caption{compute topics for each single cell}\label{alg3}
\end{algorithm}
...
\begin{algorithm}
   ...
   $computedMaps \longleftarrow$ maps computed before}
    ...
    \Fn{\CompComArea{}}{
        ...
        \tcc{compute intersection between old maps and new map}
        \For{\forRecCom}{
